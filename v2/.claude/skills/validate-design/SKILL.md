...
10. Append numbered `BLOCKER`, `SHOULD`, and `NOTE` findings. Record evidence and resolution without deleting the original concern.
11. Obtain the required human design decision for `HIGH` and `CRITICAL` work. Hand off to `/develop-code` in a fresh session.
11. Record the validated design, set `Design outcome: DESIGN-VALIDATED`, and set stage `DESIGN-VALIDATED` only when no blocker remains. Use `Design outcome: BLOCKED` and preserve stage `PLANNED` otherwise.

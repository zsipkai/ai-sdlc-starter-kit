...
10. Record reviewer identities, findings, fixes, retests, remaining risks, and human acceptance decisions. Append the stage's spend line, then hand off to `/test-regression` in a fresh session — the full matrix must certify the exact reviewed candidate.

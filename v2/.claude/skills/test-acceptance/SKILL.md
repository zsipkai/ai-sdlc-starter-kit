...
Test authoring runs at the record's tier, capped at `mid` (routing
rule 2) — delegated the same way as /develop-code, with a stated return
budget. The test diff still gets top-tier scrutiny in /clear-review.
...
10. Every numeric claim in the evidence (counts, distributions, totals) comes from a quoted command run at evidence time — subagent self-reports are leads, not evidence.
11. Set `Acceptance outcome: ACCEPTANCE-TEST-PROVEN` and stage `ACCEPTANCE-TEST-PROVEN` only when every criterion has credible evidence. Append the stage's spend line, then hand off to `/clear-review` in a fresh session.

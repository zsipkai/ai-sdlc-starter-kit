...
## Applying the routed tier

The record's `Execution model:` is applied by delegation: the top-tier
orchestrator spawns the implementation as a subagent with that model
override and the matching effort level, hands it only the change record
(the isolation test means it needs nothing else), and states a return
budget — raw structured data back, never narrative. Grep before reading;
filter tool output before it enters context. Parallel workers only for
disjoint mechanical batches on the small tier. Escalation: a blocker or
twice-red gate re-runs the stage one tier up, noted in the record — never
improvise through it, never self-downgrade.
...
10. Append actual scope, changed files, dependency provenance, authority used, decisions, test results, documentation updates, and deviations to the Implementation Record. Set `Development outcome: DEVELOPED` and stage `DEVELOPED`. Append `Tokens spent (develop-code): <n>` to the record's spend ledger, then hand off to `/test-acceptance` in a fresh session.

...
while IFS= read -r record; do
  ...
  grep -Eq '^Size: (S|M|L|XL|XXL)$' "$record" \
    || report_fail "$record needs one valid size (S|M|L|XL|XXL)"
  grep -Eq '^Execution model: (small|mid|top)' "$record" \
    || report_fail "$record needs an execution tier (small|mid|top)"

  # Model-routing floor (docs/rules/model-routing-rules.md rule 7):
  # HIGH and CRITICAL risk must execute on the top tier — enforced here
  # so no reviewer has to catch it.
  risk_val=$(sed -n 's/^Risk:[[:space:]]*//p' "$record" | head -n 1 | awk '{print $1}')
  tier_val=$(sed -n 's/^Execution model:[[:space:]]*//p' "$record" | head -n 1 | awk '{print $1}')
  case "$risk_val" in
    HIGH|CRITICAL)
      [ "$tier_val" = "top" ] \
        || report_fail "$record: Risk $risk_val requires 'Execution model: top' (got '${tier_val:-<none>}') — routing rule 7"
      ;;
  esac
...
done < <(find "${CHANGE_RECORDS_DIR:-docs/changeRecords}" -type f -name 'STORY-*.md' -print | sort)

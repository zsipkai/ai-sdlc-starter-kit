...
Every story carries **Size** (S–XXL — how much work, sets the token
budget) and **Risk** (LOW..CRITICAL — how wrong it can go, selects the
executing model tier). `/plan-task` assigns the tier; `/validate-design`
checks it; the change-record gate mechanically rejects HIGH/CRITICAL
work routed below `top`. The principle in one line: *generation may be
cheap only while verification stays expensive* — verification stages
always run the top tier. The full policy — tiers, routing table,
escalation, budgets, and the context/output economy rules (prefix
stability, fresh session per stage, effort follows the tier, input
diet, fan-out-to-judge) — lives in `docs/rules/model-routing-rules.md`.
`scripts/cost-report.sh` aggregates every record's spend: cost claims
come from that report, never from memory.

...
10. Route the work (docs/rules/model-routing-rules.md): set `Size` and `Risk` on the card and record, assign `Execution model: small | mid | top` from Risk and ambiguity — never Size alone — and set `Budget:` from the size table. An XXL story is split before this stage ends, never planned as one record.
11. Link the record from the story, move the story to `IN DELIVERY`, set `Planning outcome: PLANNED`, and set the record stage to `PLANNED`. Hand off to `/validate-design` in a fresh session — the record carries everything (routing rule 12).
